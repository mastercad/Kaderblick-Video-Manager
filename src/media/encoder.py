...
def build_encoder_args(encoder: str, preset: str, crf: int,
    ...
    if encoder == "h264_nvenc":
        ...
        args += build_h264_compatibility_args()
        if use_fps:
            args += ["-r", fps_str]
        if no_bframes:
            args += ["-bf", "0"]
    ...
    args = ["-c:v", "libx264", "-preset", p, "-crf", str(c), *build_h264_compatibility_args()]
    if use_fps:
        args += ["-r", fps_str]
...
def build_h264_compatibility_args() -> list[str]:
    """Gemeinsame H.264-Ausgabeparameter für breite Player-Kompatibilität."""
    return [
        "-pix_fmt", "yuv420p",
        "-profile:v", _H264_PROFILE,
    ]


def build_aac_audio_args(bitrate: str,
                         *,
                         sample_rate: int = 48000,
                         channels: int = 2) -> list[str]:
    """AAC-LC Audio mit konservativen Player-kompatiblen Defaults."""
    return [
        "-c:a", "aac",
        "-profile:a", "aac_low",
        "-b:a", bitrate,
...
def build_mp4_output_args(*, faststart: bool = True) -> list[str]:
    """Konservative MP4-Muxer-Parameter gegen Timestamp- und Playback-Probleme."""
    args = [
        "-map_metadata", "-1",
        "-map_chapters", "-1",
        "-dn",
